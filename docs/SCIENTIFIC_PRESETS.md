...
The two preset IDs are unchanged. Existing saves keep their bodies and assumptions;
...
moon displays expand separation to match their parent's displayed size. Guided views fit
the relevant bodies, hide the grid for compact views, and dismiss the phone outliner.
Simulation time is saved so analytic satellites reload at the saved phase; older saves
default to elapsed time zero.
